...
**Automating it** — `npm run reg:watch` (`scripts/watch-registrations.mjs`) keeps the number current without manual `reg:set` calls. It's **browserless** — plain `fetch`, no Playwright. The site is Joomla, so it logs in via the standard `com_users` form (`POST /login?task=user.login` with `username`/`password`, the per-session 32-hex CSRF token, and `remember=yes`), then `GET`s the roster and parses the total out of the `#filter-bar .btn` text ("Results 1 – 50 of N", tags stripped before matching). Every 30 min; validates the number and only writes the blob on change.

Login happens with a password **at most once**: the cookie jar (incl. Joomla's Remember Me cookie) persists, so subsequent cycles and restarts reuse the session or are silently re-authed by the remember-me cookie. Each cycle logs its path: `session reused`, `re-auth (remember-me)`, or `full login (password)`. Creds in `.env`: `EVERGREEN_USERNAME` / `EVERGREEN_PASSWORD`.

**Local + cloud share one core.** The browserless logic lives in `scripts/lib/roster.mjs` (`createRosterSync`), with the cookie store and blob store injected:
- **Local** — `scripts/watch-registrations.mjs` (`npm run reg:watch`): a 30-min loop, cookie jar in `scripts/.auth/` (git-ignored), blob written from outside Netlify (needs `NETLIFY_AUTH_TOKEN` + `NETLIFY_SITE_ID`). Run this at your laptop.
- **Cloud** — `netlify/functions/sync-registrations.mjs`: a **Netlify Scheduled Function** (`export const config = { schedule: '*/30 * * * *' }`). Same core, but the cookie jar is a blob (`evergreen-cookies` key in the `event` store) so it survives ephemeral runs, and Blobs auto-configure in the runtime (no Netlify token). Set `EVERGREEN_USERNAME` / `EVERGREEN_PASSWORD` as **Netlify site env vars**. Runs unattended on every deploy of `main`; test with `netlify functions:invoke sync-registrations`. In cron mode the short session is always stale between runs, so expect `re-auth (remember-me)` each time (no password) rather than `session reused`.
